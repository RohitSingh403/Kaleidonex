...
export function AdminSidebar({
  ...
  return (
    <aside className="hidden h-screen sticky top-0 w-64 shrink-0 flex-col border-r border-border bg-card md:flex select-none z-20 overflow-hidden">
      ...
      <div className="shrink-0 px-5 pt-5 pb-3 border-b border-border/70 bg-card">
        ...
        <div className="mt-2 flex items-center gap-2">
          ...
          <p className="text-xs font-semibold uppercase tracking-wider text-accent">
            {scope === "ceo" ? "CEO Console" : scope === "hr" ? "HR Console" : "Employee Workspace"}
          </p>
        </div>
      </div>

      {/* Scrollable Navigation Groups */}
      <nav className="flex-1 overflow-y-auto overflow-x-hidden p-3 space-y-5 scrollbar-thin scrollbar-thumb-muted-foreground/20 hover:scrollbar-thumb-muted-foreground/40">
        {groups.map((g) => (
          <div key={g.title} className="space-y-1">
            <p className="px-3 pb-1 text-[11px] font-bold uppercase tracking-wider text-muted-foreground/90">
              {g.title}
            </p>
            {g.items.map((id) => {
